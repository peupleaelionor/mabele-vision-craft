    ...
    <div className="container-edge relative py-20 sm:py-28">
      <div className="max-w-2xl">
        ...
        <h2 className="mt-6 font-serif text-4xl sm:text-6xl leading-[1] tracking-tight">
          Comment puis-je vous aider <span className="italic text-accent">aujourd'hui</span> ?
        </h2>
        <p className="mt-6 text-base sm:text-lg text-primary-foreground/75 max-w-xl">
          Posez une question, dérivez ce dont vous avez besoin. Mabele répond clairement et vous aide à passer à l'action.
        </p>

        <div className="mt-10 rounded-2xl bg-background/8 backdrop-blur border border-primary-foreground/15 p-2 flex items-center gap-2">
          <div className="flex-1 px-4 py-3 text-sm text-primary-foreground/70">Demandez à Mabele…</div>
          <Link to="/chat" className="inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-3 text-sm text-accent-foreground hover:opacity-95">
            Ouvrir <ArrowUpRight className="h-4 w-4" />
          </Link>
        </div>

        <div className="mt-6 flex flex-wrap gap-2">
          {["Générer une stratégie", "Analyser un marché", "Rédiger un plan", "Synthèse de document"].map((s) => (
            <Link key={s} to="/chat" className="rounded-full border border-primary-foreground/20 px-4 py-2 text-xs text-primary-foreground/85 hover:bg-primary-foreground/5 transition">
              {s}
            </Link>
          ))}
        </div>
